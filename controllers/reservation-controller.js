                            ...
                            if(err){
                                ...
                                Room.findById(roomId,(err,roomFinded)=>{
                                    if(err){
                                        ...
                                        if(roomFinded.available_day == null){
                                            var confirmedRoom = false;
                                            hotelFinded.rooms.forEach(element => {
                                                if(element == roomFinded._id){
                                                    confirmedRoom = true;
                                                }
                                            });
                                            if(confirmedRoom == true){
                                                if(roomFinded.available == true){
                                                    ...
                                                    reservation.save((err,reservationSaved)=>{
                                                        if(err){
                                                            return res.status(500).send({message: "Error al guardar reservación"});
                                                        }else if(reservationSaved){
                                                            var userReserve = reservationSaved._id;
                                                            Hotel.findByIdAndUpdate(hotelId,{count_reservations: count},(err,hotelUpdated)=>{
                                                                if(err){
                                                                    return res.status(500).send({message: "Error al actualizar conteo"});
                                                                }else if(hotelUpdated){
                                                                    Room.findByIdAndUpdate(roomId,{available: false, available_day: end},(err,roomUpdated)=>{
                                                                        if(err){
                                                                            return res.status(500).send({message: "Error al deshabilitar habitación"});
                                                                        }else if(roomUpdated){
                                                                            User.findByIdAndUpdate(userId,{$push:{reservations: userReserve, history_hotels: userHotel}},(err,userUpdated)=>{
                                                                                if(err){
                                                                                    return res.status(500).send({message: "Error al actualizar registro de usuario"});
                                                                                }else if(userUpdated){
                                                                                    return res.send({message: "Ha reservado la habitación exitosamente", reservationSaved});
                                                                                }else{
                                            ...
                                            return res.send({message: "La habitación estará disponible hasta el día",availableDay});
                                        }
                                    }else{
...
function cancelReservation(req,res){
    ...
    Reservation.findById(reservationId,(err,reservationFinded)=>{
        if(err){
            ...
            if(userId == req.user.sub || req.user.role == "ROLE_ADMIN" || req.user.role == "ROLE_HOTEL"){
                Reservation.findByIdAndRemove(reservationId,(err,reservationRemoved)=>{
                    if(err){
                        ...
                        User.findByIdAndUpdate(userId,{$pull:{reservations: reservationId, history_hotels: hotelId}},(err,userUpdated)=>{
                            if(err){
                                    ...
                                    if(err){
                                        ...
                                        Hotel.findByIdAndUpdate(hotelId,{count_reservations: count},(err,hotelUpdated)=>{
                                            if(err){
                                                ...
                                                Room.findByIdAndUpdate(roomId,{available: true, available_day: null},(err,roomUpdated)=>{
                                                    if(err){
