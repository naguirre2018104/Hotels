            ...
            Reservation.findById(reservationId,(err,reservationFinded)=>{
                if(err){
                    ...
                    if(reservationFinded.status == true){
                        var start = new Date(moment(reservationFinded.start_date).format("YYYY-MM-DD"));
                        var today = new Date(moment().format("YYYY-MM-DD"))
                        if(today <= start){
                            return res.send({message: "Aún no es día de retiro, no se puede pagar"});
                        }else{
                            Room.findByIdAndUpdate(roomId,{available: true,available_day: null},{new:true},(err,roomUpdated)=>{
                                if(err){
                                    return res.status(500).send({message: "Error al buscar habitación"});
                                }else if(roomUpdated){
                                    Reservation.findByIdAndUpdate(reservationId,{status: false},{new: true},(err,reservationUpdated)=>{
                                        if(err){
                                            ...
                                            invoice.save((err,invoiceSaved)=>{
                                                if(err){
                                                    ...
                                                    return res.send({message: "Facturado exitosamente",invoiceSaved});
                                                }else{
...
function getInvoice(req,res){
    ...
    Hotel.findOne({user_admin_hotel: userId},(err,hotelFinded)=>{
        if(err){
            ...
            var hotelId = hotelFinded._id;
            console.log(hotelId);
            Invoice.findById(invoiceId).populate("services").exec((err,invoice)=>{
                if(err){
...
function getInvoicesByUser(req,res){
    ...
    Invoice.find({user: userId}).exec((err,invoices)=>{
        if(err){
            return res.status(500).send({message: "Error al obtener facturas"});
        }else if(invoices){
            return res.send({message: "Facturas: ",invoices});
